...
class Weather(commands.Cog):
    ...
    def get_weather(self, location):
        ...
        currently_field = '{} {}.'.format(
                weather["current_icon"],
                weather["current_summary"]
                )
        if "hourly_summary" in weather:
            currently_field += ' {}'.format(
                    weather["hourly_summary"]
                    )
        if "daily_summary" in weather:
            currently_field += ' {}'.format(
                    weather["daily_summary"]
                    )

        temp_field = '{:,g}°C ({:,g}°F)'.format(
                weather["current_temp"]["c"],
                weather["current_temp"]["f"],
                )

        temp_feels_field = '{:,g}°C ({:,g}°F)'.format(
                weather["feels_like"]["c"],
                weather["feels_like"]["f"],
                )

        temp_high_low_field = '{:,g}°C ({:,g}°F)/{:,g}°C ({:,g}°F)'.format(
                weather["daily_temp"]["high"]["c"],
                weather["daily_temp"]["high"]["f"],
                weather["daily_temp"]["low"]["c"],
                weather["daily_temp"]["low"]["f"]
                )
